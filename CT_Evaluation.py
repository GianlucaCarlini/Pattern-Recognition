# -*- coding: utf-8 -*-
"""
Created on Wed Apr 15 13:26:12 2020
...
for i in range(len(labels_CT)):
    if labels_CT[i] > 1:
        labels_CT[i] = 1

features_CT, correlated_CT = Remove_Correlated(df_features, features_CT, verbose = True)

features_CT = Lasso_Selection(features_CT, labels_CT)

features_array = features_CT.to_numpy()

features_forest, idx = Forest_Selection(features_array, labels_CT,
                                                            how_many = 15)
        
X_train, X_test, Y_train, Y_test = train_test_split(features_forest, labels_CT, test_size = 0.3,
                                                    random_state = 7)
...
forest = RandomForestClassifier(max_features = 7, max_leaf_nodes = 128, n_estimators = 100)
...
def plot_roc_curve(fpr, tpr, score):
    ...
    plt.title('CT Images ROC curve', fontsize = 'x-large')
